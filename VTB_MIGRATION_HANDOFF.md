...
- **`idleMessages[14]` が到達不能だった件は修正済み。**
  配列は 15件（index 0〜14）あるが `idleLoopEnd = 13` が直書きされていたため、
  index 13 の次は 1 に戻り index 14（「あ、もしかして案内を読み飛ばしちゃったのかな？
  じゃあもう一回説明しよう！」）が一度も表示されていなかった。
  ユーザー確認のうえ `const idleLoopEnd = idleMessages.length - 1;` に変更し、
  `0 → 1 → … → 14 → 1 → …` と巡回することを確認済み。
  **移植時もセリフ件数から求める形を維持すること**（直書きするとセリフを足した時に
  同じ事故が再発する）。
